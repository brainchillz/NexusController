...
# ─── controller-specific: the callback is registered but inert ─────────

def test_callback_inert_and_admin_gated_endpoints(monkeypatch):
    """The callback exists unconditionally (so UI enrollment applies without a
    restart) but must behave as absent while unconfigured. The settings
    endpoints are ADMIN_ONLY here, not merely authenticated."""
    import app as ctl
    c = ctl.app.test_client()
    assert c.get('/sso/callback?a=x').status_code == 404
    for call in (lambda: c.get('/api/sso'),
                 lambda: c.post('/api/sso/enroll', json={'issuer': 'https://x', 'code': 'y'}),
                 lambda: c.delete('/api/sso', json={'password': 'x'})):
        assert call().status_code == 401
    assert {'sso_status', 'sso_enroll', 'sso_disable'} <= ctl.ADMIN_ONLY


def test_assertion_carries_no_scope(configured):
    """The controller is the one app with a scope model. An assertion must not
    be able to smuggle one in — verify returns a subject and nothing else."""
    assert configured.verify(tok('valid'), now=NOW) == 'admin'
    import json as _json
    payload = _json.loads(sso._b64u_decode(tok('valid').split('.')[1]))
    assert 'scope' not in payload and 'scope_tags' not in payload and 'role' not in payload
